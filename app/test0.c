...
int main(int argc, char *argv[])
{
	...
	// test write
	printf("\r\nMaster>RPU0 writting test ...\r\n");
	gettimeofday(&start,NULL);
	while(1){
		set_cmd.cmd_id = RPU_SET_PARAMS;
		sprintf(set_cmd.data, "%s%d%s", "hello RPU0, i am send cmd [", cnt, "]");
	...
	while(1){
		...
		if(cnt == 1000){
			...
			usedTime = (double)(end.tv_sec-start.tv_sec)+ ((double)(end.tv_usec-start.tv_usec))/1000000;
			speed = (double)(sizeof(get_cmd) * cnt) / (usedTime) /(1024.0);
			printf("\r\nMaster<RPU1 reading test  done !!, size = %d, usedTime : %.4f s\t(%.2f MB/s)\r\n",sizeof(get_cmd) * cnt, usedTime, speed);			cnt = 0;
			break;
		}
	}
	
	#if 1
	printf("\r\nMaster<-->RPU0 write and read test ...\r\n");
	while(1){
		
		set_cmd.cmd_id = RPU_SET_PARAMS;
		sprintf(set_cmd.data, "%s%d%s", "RPU0 write / read test ... [", cnt, "]");
		ioctl(rpmsg_device->rpmsg_dev_fd, RPU_IOC_SET, &set_cmd);
		...
		cnt++;
		if(cnt == 100){
			printf("\r\nMaster<-->RPU0 write and read test done !!\r\n");
